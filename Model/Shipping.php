...
namespace Informatics\Restrictproduct\Model;
...
class Shipping extends \Magento\Shipping\Model\Shipping
{
     ...
     * @return bool
     */
    public function isShippingRestriction(\Magento\Quote\Model\Quote\Address\RateRequest $request)
    {
        $options         = array();
        $shippingCountry = $request->getDestCountryId();
        /*$debugMsg1 = "=============== Informatics_Restrictproduct Module -1  =============== \n Selected country is for shipping is " . $shippingCountry . ".";
        $this->_logger->debug($debugMsg1);*/
        ...
        foreach ($request->getAllItems() as $quoteItem) {
            ...
            $itemDisallowcountries = explode(',', $quoteItem->getProduct()->getShippingRestriction());
            /*$debugMsg2 = "=============== Informatics_Restrictproduct Module -2  =============== \n Restricted country of the products are " . $quoteItem->getProduct()->getId() ."<=>". $quoteItem->getProduct()->getShippingRestriction() . ".";
            $this->_logger->debug($debugMsg2);*/

            /*$debugMsg3 = "=============== Informatics_Restrictproduct Module -3  =============== \n Option array empty => " . print_r($options) ."<=>.";
            $this->_logger->debug($debugMsg3);*/
            foreach ($itemDisallowcountries as $ccodeItem) {
                /*$debugMsg4 = "=============== Informatics_Restrictproduct Module -4  =============== \n Country name from forloop => " . $ccodeItem ."<=>.";
                $this->_logger->debug($debugMsg4);*/
                if ($shippingCountry == $ccodeItem) {

                    $options[] = $quoteItem->getProduct()->getName();
                }
            }
        }
        
        /*$debugMsg5 = "=============== Informatics_Restrictproduct Module -5  =============== \n Final options array => " . print_r($options) ."<=>.";
        $this->_logger->debug($debugMsg5);*/
        return $options;
    }
    ...
    {
        $check = $this->isShippingRestriction($request);
        if(count($check) > 0) { //if (!empty($request)) {
            $arrayRestrict = (array_unique($check));
            //$shippingCountry = $request->getDestCountryId();
            $blockedProduct = implode(',', $arrayRestrict); //"test";# $blockedProduct = implode(',', $arrayRestrict);

            $debugMsg = "=============== Informatics_Restrictproduct Module =============== \n Selected products " . $blockedProduct . " not shipping to your Shipping country.";
            $this->_logger->debug($debugMsg);
            /*
            $message = 'Selected products ' . $blockedProduct . ' not shipping to your Shipping country. Change your shipping country';
            $this->_messageManager->addError($message);
            */
            throw new StateException(__('Selected product(s) %1 not shipping to your Shipping country. Change your shipping country.',  $blockedProduct));
            //throw new LocalizedException(__('Selected products %1 not shipping to your Shipping country. Change your shipping country.',  $blockedProduct));
            //return $this;
        }
